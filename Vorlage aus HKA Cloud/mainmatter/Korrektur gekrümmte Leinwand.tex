...
\begin{center}
	...
	\end{tikzpicture}
	\captionof{figure}[Visualisierung der verzerrenden Wirkung einer gekrümmten Leinwand auf die Ergebnisse des Eye Trackers]
	{Visualisierung der verzerrenden Wirkung einer gekrümmten Leinwand auf die Ergebnisse des Eye Trackers}
	\captionsetup{justification=justified, singlelinecheck=false}
	{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
...
\begin{figure}
	\begin{center}
		...
		a.) - Direkte Blickrichtung auf Mitte der Leinwand (0° Winkel)\\
		b.) - Vom Eye Tracker angenommene Blickrichtung zum äußeren linken Rand einer imaginär flachen Leinwand\\
		c.) - Tatsächliche Blickrichtung zum äußeren linken Rand der realen, gekrümmten Leinwand
		}
		 \label{fig:kruemmungsproblem_1}
	\end{center}
	% Das Label wird gebraucht, um im Text auf diese Abbildung zu referenzieren
...
In Abbildung \ref{fig:kruemmungsproblem_1} ist die gekrümmte Leinwand, sowie eine flache Leinwand zu erkennen. Aus Sicht des Eye Trackers existiert nur die flache Leinwand. Ein grauer Punkt kennzeichnet die Position der Testperson im Fahrsimulator. Pfeil a.) zeigt direkt von der Testpostion in einem Winkel von 0° auf die Mitte der Leinwand. Die Abbildung verdeutlicht, dass es in diesem Punkt keine Abweichung zwischen der flachen Leinwand und der gekrümmten Leinwand gibt. Aus diesem Grund ist zu erwarten, dass der Eye Tracker in diesem Punkt die genausten Ergebnisse liefert. 
...
\begin{center}
	\begin{tikzpicture}
		...
		\end{tikzpicture}
<<<<<<< HEAD
		\captionof{figure}[Visualisierung der nichtlinearen Transformation]{Visualisierung der nichtlinearen Transformation}
=======
		\caption[]{Visualisierung der nichtlinearen Transformation}
>>>>>>> fd0befd8e95e2bae230d21ece8a1862eada6b577
		\captionsetup{justification=justified, singlelinecheck=false}
		{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
		...
		$\alpha$ - Winkel zwischen Blickrichtung von 0° und Pfeil a.)\\
		$\beta$ - Winkel zwischen Blickrichtung von 0° und Pfeil c.)} \label{fig:kruemmungsproblem_2}
\end{center}
...
\begin{center}
	...
	\end{tikzpicture}
	\captionof{figure}[Visualisierung der Verschiebung der Sitzposition der Testperson]{Visualisierung der Verschiebung der Sitzposition der Testperson}
	\captionsetup{justification=justified, singlelinecheck=false}
	{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
	...
	f.) - Transformation von Pfeil e.) auf die imaginäre flache Leinwand} \label{fig:kruemmungsproblem_3}
\end{center}
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
		\caption[]{Visualisierung der Verschiebung der Sitzposition der Testperson}
		\captionsetup{justification=justified, singlelinecheck=false}
		{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
		a.) - Blickrichtung auf den äußeren linken Rand der realen gekrümmten Leinwand\\
		b.) - Transformation von Pfeil a.) auf die imaginäre flache Leinwand\\
...
\begin{center}
	...
	\end{tikzpicture}
	\captionof{figure}[Graphische Darstellung des Lösungsansatzes für die Transformation]{Graphische Darstellung des Lösungsansatzes für die Transformation}
	\captionsetup{justification=justified, singlelinecheck=false}
	{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
...
\begin{center}
	...
	\end{tikzpicture}
	\captionof{figure}[Bestimmung der notwendigen Korrektur auf beiden Halbseiten des Halbkreises]{Bestimmung der notwendigen Korrektur auf beiden Halbseiten des Halbkreises}
	\captionsetup{justification=justified, singlelinecheck=false}
	{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
		\caption[]{Graphische Darstellung des Lösungsansatzes für die Transformation}
		\captionsetup{justification=justified, singlelinecheck=false}
		{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
		a.) - Blickrichtung auf die Mitte der realen gekrümmten Leinwand\\
		b.) - Vom Eye Tracker erkannte Blickrichtung auf der imaginären geraden Leinwand, wenn Testperson in Blickrichtung d.) auf gekrümmter Leinwand schaut\\
		c.) - Korrekte Transformation von gekrümmter Leinwand auf gerade Leinwand\\
		d.) - Blick der Testperson auf einen Punkt auf der gekrümmten Leinwand\\
		$\alpha$ - Winkel zwischen Blickrichtung zur Mitte der realen gekrümmten Leinwand und aktueller Blickrichtung\\
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
		\caption[]{Bestimmung der notwendigen Korrektur auf beiden Halbseiten des Halbkreises}
		\captionsetup{justification=justified, singlelinecheck=false}
		{\footnotesize Grauer Punkt - Sitzposition der Testperson\\
		a.) - Blickrichtung auf einen Punkt auf der linken Hälfte der gekrümmten Leinwand\\
		b.) - Korrekte Transformation von Blickrichtung a.) auf die imaginäre gerade Leinwand\\
...
Dabei ist: 
\begin{center}
	k - Korrektur-Wert in Pixeln\par
...
\begin{center}
	\includegraphics[scale=0.5]{e_function_param}
	\captionof{figure}{Sinnvolle Werte für den Korrekturfaktor linke Bildschirmhälfte} \label{fig:imag6}
\end{center}
...
Anstatt einer e-Funktion wird eine \Gls{quadratische Funktion} eingesetzt. Das $k_r$ ist wie bei der e-Funktion nur auf der rechten Bildschirmhälfte anzuwenden. 
...
\begin{center}
	...
	\captionof{figure}{Visualisierung quadratische Korrekturfunktion, Quelle: \cite{lit_software_matlab}} \label{fig:imag8}
\end{center}

=======
Abbildung \ref{fig:imag8} zeigt, dass im Vergleich zur e-Funktion eine stärkere Korrektur in der Nähe der Mitte der Leinwand stattfindet. An den Rändern ist der Korrekturwert nicht so hoch wie bei der e-Funktion, wodurch der nutzbare Sichtbereich weniger stark eingeschränkt wird. 

Im Fahrsimulator konnte eine leichte Verbesserung nachgewiesen werden. Hierzu wurde eine Auswertung der Ergebnisse durchgeführt. 
>>>>>>> fd0befd8e95e2bae230d21ece8a1862eada6b577
...
\begin{center}
	\includegraphics[scale=0.3]{first_correction_function_results}
	\captionof{figure}{Gegenüberstellung reale Blickposition und Eye Tracker-Werte: unkalibriert (links), kalibriert (rechts), Quelle: \cite{lit_software_matlab}} \label{fig:imag9}
\end{center}
...
\begin{figure}[h!]
	\begin{center}
		\includegraphics[scale=0.3]{first_correction_function_results_zoomed}
...
\begin{center}
	\includegraphics[scale=0.3]{first_correction_function_results_zoomed}
	\captionof{figure}{Gegenüberstellung reale Blickposition und Eye Tracker-Werte im Detail: unkalibriert (links), kalibriert (rechts), Quelle: \cite{lit_software_matlab}} \label{fig:imag10}
\end{center}
...
\begin{center}
	\includegraphics[scale=0.5]{second_correction_function_param}
	\captionof{figure}{Visualisierung V-Korrekturfunktion, Quelle: \cite{lit_software_matlab}} \label{fig:imag11}
\end{center}
